    ...
    >
      ...
      <div className="container mx-auto p-4 relative z-10">
        ...
        <div className="grid grid-cols-12 gap-6">
              ...
              <CardContent className="p-4">
                <nav className="space-y-2">
                  ...
                  <NavItem
                    icon={Database}
                    label="Data Center"
                    active={currentView === "data-center"}
                    onClick={() => setCurrentView("data-center")}
                  />
                  <NavItem
                    icon={Globe}
                    label="Network"
          ...
          {/* Main content area */}
          <div className="col-span-12 md:col-span-9 lg:col-span-7">{renderView()}</div>
...
function AlertItem({
  ...
  const getIcon = () => {
    switch (type) {
      case "info":
        return <AlertCircle className="h-4 w-4 text-blue-500 mr-2" />
      case "warning":
        return <AlertCircle className="h-4 w-4 text-amber-500 mr-2" />
      case "update":
        return <Download className="h-4 w-4 text-cyan-500 mr-2" />
      case "success":
...
function CommunicationItem({
  ...
  return (
    <div className="flex items-start relative">
      ...
      {unread && <span className="absolute top-0 right-0 h-2 w-2 bg-blue-500 rounded-full"></span>}
    </div>
  )
}
...
function ActionButton({ icon: Icon, label }: { icon: LucideIcon; label: string }) {
  return (
    <Button className="w-full justify-center bg-slate-800/50 hover:bg-slate-700/50 text-sm text-slate-400 border border-slate-700/50">
      <Icon className="mr-2 h-4 w-4" />
      {label}
    </Button>
  )
